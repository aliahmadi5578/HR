...
const EditProductForm = ({ initialValues, onFinish, categories }) => {
  ...
  useEffect(() => {
    if (initialValues) {
      const isSamePrice = initialValues.staff_price === initialValues.price;
      
      setFormData({
        stock_name: initialValues.stock_name || '',
        price: initialValues.price?.toString() || '',
        staff_price: initialValues.staff_price?.toString() || '',
        category_id: initialValues.category_id?.toString() || '',
        is_active: initialValues.is_active !== undefined ? initialValues.is_active : true,
        use_same_price: isSamePrice
      });
      
      setUseSamePrice(isSamePrice);
    }
  }, [initialValues]);
  ...
  const validateForm = () => {
    ...
    if (formData.staff_price && (isNaN(formData.staff_price) || Number(formData.staff_price) < 0)) {
    ...
    <Form onSubmit={handleSubmit}>
      <Row>
        <Col md={12} className="mb-3">
          <Form.Group>
            <Form.Label>نام محصول <span className="text-danger">*</span></Form.Label>
            <Form.Control
              type="text"
              name="stock_name"
        ...
        <Col md={6} className="mb-3">
          <Form.Group>
            <Form.Label>قیمت (ریال) <span className="text-danger">*</span></Form.Label>
            <Form.Control
              type="text"
              name="price"
        ...
        <Col md={6} className="mb-3">
          <Form.Group>
            ...
            <Form.Control
              type="text"
              name="staff_price"
              value={useSamePrice ? Comma(formData.price) : Comma(formData.staff_price)}
              onChange={handlePriceChange}
